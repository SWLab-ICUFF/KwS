...
public class Graph {
   ...
   /**
    ...
    public int degree(String v) {
        if (!st.containsKey(v)) throw new RuntimeException(v + " is not a vertex");
        else return st.get(v).size();
    }

    /**
     * Add edges v-w and w-v to this graph (if it is not already an edge) with cost 1
     */
   ...
   /**
     * Add edge v-w and w-v to this graph (if it is not already an edge) with a cost associated
     */
    ...
    /**
     * Add edge v-w to this graph (if it is not already an edge) with a cost associated
     */
     ...
     * CAUTION!! this method will not remove the edges that points to v
     */
   ...
   /**
    ...
    public void addVertex(String v) {
        if (!hasVertex(v)) st.put(v, new HashMap<String, Integer>());
    }
   ...
   /**
    ...
    public Iterable<String> adjacentTo(String v) {
        ...
        if (!hasVertex(v)) return new ArrayList<String>();
        else               return new ArrayList<String>(st.get(v).keySet());
    }
    ...
    public HashMap<String, Integer> adjacentTo(String v, boolean withcosts) {
        // return empty set if vertex isn't in graph
        if (!hasVertex(v)) return new HashMap<String, Integer>();
